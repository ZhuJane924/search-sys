...
public interface AuthorPaperMapper {
    ...
    @Select("SELECT COUNT(1) FROM author_paper WHERE author_id = #{authorId} AND paper_id = #{paperId}")
    int countByAuthorIdAndPaperId(@Param("authorId") Long authorId, @Param("paperId") Long paperId);

    @Insert("INSERT INTO author_paper(author_id, paper_id) VALUES(#{authorId}, #{paperId})")
